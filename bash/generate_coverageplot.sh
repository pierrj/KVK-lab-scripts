...
awk -v OFS='\t' 'NR==FNR{a[$2]=$1;next}{$1=a[$1];}1' tmp.chrom_count_and_names ${SAMPLE}.genomecoverage.normalized.bed > ${SAMPLE}.genomecoverage.normalized.renamed.bed
echo -e 'CHROMOSOME''\t''BASE''\t''COUNT' > tmp.first_row
cat tmp.first_row ${SAMPLE}.genomecoverage.normalized.renamed.bed > ${SAMPLE}.genomecoverage.table
