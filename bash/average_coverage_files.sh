...
while read density_file; do
    ...
    awk -v N=$sum_rpks '{print $1/N}' ${basename_density_file}.${LABEL}.coverage.RPK > ${basename_density_file}.${LABEL}.coverage.normalized
done < $BAMFILE_MAPFILE
paste *.${LABEL}.coverage.normalized | awk '{sum = 0; for (i = 1; i <= NF; i++) sum += $i; sum /= NF; print sum}' > ${LABEL}.${OUTPUT_NAME}.txt