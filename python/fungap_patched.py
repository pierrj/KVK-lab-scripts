...
def get_paths():
    ...
    d_path['run_braker_patched'] = os.path.join(this_dir, 'run_braker_patched.py')
...
def run_braker(
    ...
    command = (
        ...
        '--translation_table {} --log_dir {} --num_cores {} {}'.format(
            d_path['run_braker_patched'], masked_assembly, ' '.join(trans_bams),
            braker_output_dir, translation_table, log_dir, num_cores,
            no_braker_fungus
        )
    )
    logger_time, logger_txt = logger
    logger_time.debug('START: wrapper_run_braker_patched PATCHED BY PIERRE')
    logger_txt.debug('[Wrapper] %s', command)
    command_args = shlex.split(command)
    check_call(command_args)
    logger_time.debug('DONE : wrapper_run_braker_patched\n')
